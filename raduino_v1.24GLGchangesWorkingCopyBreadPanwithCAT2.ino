/** FT-857d CAT frquency control added  **/
...
#define MIN_FREQ 3300000UL        // absolute minimum tuning frequency in Hz
#define MAX_FREQ 7300000UL        // absolute maximum tuning frequency in Hz
#define ILLEGAL_BOTTOM 4000000UL  // bottom end of no-mans land
#define ILLEGAL_TOP    6800000UL  // top end of no-mans land
...
#define VFO_DRIVE_USB 6           // VFO drive level in USB mode in mA [accepted values 2,4,6,8 mA]
...
#define SI5351BX_XTAL 25004000          // Crystal freq in Hz  breadpan version 460
  ...
  if (knob < 3) {
    if (frequency > LOWEST_FREQ) {
      if (POT_SPAN < 25)
        ...
        baseTune = baseTune - 25000UL; // mod fast tune down in 25 kHz steps
      frequency = baseTune + (unsigned long)knob * (unsigned long)POT_SPAN / 10UL;
      if (clicks < 10)
        printLine(1, (char *)"<<<<<<<"); // tks Paul KC8WBK

        // move quickly through the non-ham frequencies between 80 and 40 meters
      if ((frequency > ILLEGAL_BOTTOM) & (frequency< ILLEGAL_TOP))
      {   delay (100); }
      else
      delay(1000);
    }
  ...
  // if the POT_SPAN is very small (less than 25 kHz) then use 1 kHz steps instead
    ...
    if (frequency < HIGHEST_FREQ) {
      if (POT_SPAN < 25)
        ...
        baseTune = baseTune + 25000UL; // mod fast tune up in 25 kHz steps
      frequency = baseTune + (unsigned long)knob * (unsigned long)POT_SPAN / 10UL;
      if (clicks < 10)
        printLine(1, (char *)"         >>>>>>>"); // tks Paul KC8WBK        
